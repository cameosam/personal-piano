...
export default function Learn(props) {
    ...
    const [subtitleLearn, setSubtitleLearn] = React.useState('press ' + songs[songNumber].value[noteNumber] + ' to start!');
    ...
    function keySelect(key){
        if(key === songs[songNumber].value[noteNumber]){
            ...
            if (songs[songNumber].value[noteNumber+1]){
                setSubtitleLearn('next key: ' + songs[songNumber].value[noteNumber+1] + ' (note: ' + (noteNumber + 2) + ')');
                setTimeout(function() {animateButton(songs[songNumber].value[noteNumber+1], 'next-key')}, 300);
            } else {
                setNoteNumber(0);
                setSubtitleLearn('nice job! press ' + songs[songNumber].value[0] + ' to restart');
            }
        }
    };
                ...
                {songs.map((song, index) => (
                    <ToggleButton
                        ...
                        onChange={(e) => {
                            ...
                            setSubtitleLearn('press ' + songs[e.currentTarget.value].value[0] + ' to start!');
                        }}
                        >
                        {song.name}
                    </ToggleButton>
